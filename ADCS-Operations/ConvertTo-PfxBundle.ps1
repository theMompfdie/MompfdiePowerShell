function ConvertTo-PfxBundle {
    <#
    .SYNOPSIS
    Converts a private key, certificate, and CA chain into a .pfx file using OpenSSL.

    .DESCRIPTION
    This function creates a PKCS#12 (.pfx) bundle by combining a private key (.key),
    a signed certificate (.cer/.crt), and a certificate chain (.crt).
    The PFX file is protected by a password which is passed securely as a SecureString.

    .PARAMETER KeyPath
    Path to the private key file (.key).

    .PARAMETER CertificatePath
    Path to the signed certificate file (.cer or .crt).

    .PARAMETER ChainPath
    Path to the certificate chain file (e.g. intermediate + root CA).

    .PARAMETER OutputPfxPath
    Path to the resulting .pfx file. Defaults to "output.pfx" in the certificate's directory.

    .PARAMETER PfxPassword
    SecureString password to protect the PFX file.

    .PARAMETER OpenSslPath
    Path to the OpenSSL executable. Default is the standard Windows location.

    .EXAMPLE
    $pw = ConvertTo-SecureString 'Secret123!' -AsPlainText -Force
    ConvertTo-PfxBundle -KeyPath 'C:\Certs\web.key' -CertificatePath 'C:\Certs\web.cer' -ChainPath 'C:\Certs\chain.crt' -PfxPassword $pw
    #>
    ...
    param (
        ...
        [Parameter()]
        [string]$OutputPfxPath = "$((Split-Path -Path $CertificatePath))\output.pfx",
    ...
    foreach ($file in @($KeyPath, $CertificatePath, $ChainPath)) {
    ...
    $opensslArgs = @(
        ...
        '-certfile', $ChainPath
        '-out', $OutputPfxPath
        '-passout', "pass:$unsecurePassword"
    )

    Write-Host "🔧 Creating PFX with OpenSSL:"
    ...
    if (Test-Path $OutputPfxPath) {
        Write-Host "✅ PFX file successfully created: $OutputPfxPath"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUYB+wqhSuYNb3AUaolpZXlYbm
# 6b6ggiT/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBTlbLkD
# 6L0KZzDMu+CjYEF42SlFcTANBgkqhkiG9w0BAQEFAASCAQBHz//nIjOeTNxJc083
# 2mnLpTbcXIHRH5Arp02mldixFPH1gdV7K0o2vMA1jRluw4GFfaBu0K7qOeUQJMnj
# oeXR01+kuzpcc6s6U8LXzGGWlYD3W+RvPsyVkJGw/mA+OQO9aT3ngrDdHrxwIccV
# mgJrrg/rlGu6SlsTlHYY+yMn3EiRQX8ATI+2SiY3bINHU7sWo9c/KC0MKwRVJE2K
# n3Vu3qElHmPwpAQ5QUJkG7iu59/4Wl3+qMdMDRiZsa/WNCoHbBiTKMzrTFXpx+HP
# F/E6fYlvKY6NMGw3+9O4YtdUXKUXnbJp4HqlWeu7RcrlrZLehHiciNTURvzO5kLg
# zi4NoYIDIDCCAxwGCSqGSIb3DQEJBjGCAw0wggMJAgEBMHcwYzELMAkGA1UEBhMC
...
# 9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1MDUxMjE3MjkyN1owLwYJKoZIhvcNAQkE
# MSIEIJE8v8S5/1FlJmXYgm43gO0e0CLBkdU17/cmSq2aW2giMA0GCSqGSIb3DQEB
# AQUABIICAFoyyIYzOtUqaVOodpj29mpFZa5UvGHYT3f3PhT+c0hqUgL/S/zAtxA2
# 9bOUdO8UmnPljUsoWrtzKcm8nzma2XREa41/uMnrKaryreOei5yWBtMCi0KwjfpJ
# dOww8BjrjF3AcTrgGhQCSq+M7lXTFJelnmiBsMkZdbVQ3EhfXrfTEwG7+NJWC+ZI
# u7CeymF+8Swq19XGcNL07vdqZNGk/iaUO079KEBrRZts4YOy0ssR6a33XfMHo2do
# VjL2bnKOxdyYi5DVyZ7wFJOj4PCUcDblXKLfrk6vodFaUhCaAEV0nI3t/h6G2DWw
# Lw3M+d+lObEr8dUi/3iEfsdVHCk6wz6ow5Wu3NVahJhqVLEqeU2Ra0qP/iuqM6Y8
# AVZoVz+jIWYN3G2rm9M9Bar+dhIHtGUldga+pZbvOWIXfS1rLgw+Ft1yOEtu52Mh
# KYRirBElL4T9pDFVuwibjovx0QCkhFe5o6wKCBwPcOrWABSJcqWa7nQ0UPNJual5
# QHoqgUOJT+b82DvP5nwM5E4KEhRZ/lAkYj/5bRYbtOYaKSy4H0tED55b4/keXteK
# fKgi1c1sbOXsPwsIGzAqElWy1N0AvpOpg1RBkQbdbJfcdKlv53RDVjZVhWExa+w3
# m6NZUjfQDGYMbfN5MJOGwq2NZQurBBHpa9XXQZRVNWpRXk0OXYP/
