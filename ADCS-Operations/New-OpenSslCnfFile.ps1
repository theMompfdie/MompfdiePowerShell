function New-OpenSslCnfFile {
    <#
    .SYNOPSIS
    Generates an OpenSSL-compatible .cnf configuration file for CSR creation.

    .DESCRIPTION
    This function creates a configuration file for OpenSSL that defines the subject and SAN (Subject Alternative Names)
    fields used to generate a certificate signing request (CSR). It supports DNS and IP entries for SANs, and outputs
    the file to a specified location.

    .PARAMETER CommonName
    The common name (CN) of the certificate subject (e.g., web server FQDN).

    .PARAMETER Country
    The two-letter ISO country code. Default is 'AT'.

    .PARAMETER State
    The state or province. Default is 'LowerAustria'.

    .PARAMETER Locality
    The city or locality. Default is 'City'.

    .PARAMETER Organization
    The organization name. Default is 'Die Mompfdies'.

    .PARAMETER OrganizationalUnit
    The organizational unit (OU). Default is 'SmartHome'.

    .PARAMETER SubjectAltNames
    An array of SAN entries, e.g., 'DNS:web01.example.com','IP:192.168.1.100'.

    .PARAMETER OutputPath
    Path where the .cnf file will be saved. Default is %TEMP%\<CommonName>.cnf.

    .EXAMPLE
    New-OpenSslCnfFile -CommonName 'web01.example.com' -SubjectAltNames @('DNS:web01.example.com','IP:10.0.0.1')
    
    .EXAMPLE
    New-OpenSslCnfFile -CommonName 'web02.example.com' -SubjectAltNames @('DNS:web02.example.com') -OutputPath 'C:\Certificates\web02.cnf'
   #>
    ...
    param (
        ...
        [string]$Country = 'AT',
        [string]$State = 'LowerAustria',
        [string]$Locality = 'City',
        [string]$Organization = 'Die Mompfdies',
        [string]$OrganizationalUnit = 'SmartHome',

        [Parameter(Mandatory = $true)]
        [string[]]$SubjectAltNames,

        [string]$OutputPath = "$env:TEMP\$CommonName.cnf"
    )
    ...
    foreach ($san in $SubjectAltNames) {
        if ($san -like 'DNS:*') {
            $dnsList += $san -replace '^DNS:', ''
        }
        elseif ($san -like 'IP:*') {
            $ipList += $san -replace '^IP:', ''
        }
    }
    ...
    foreach ($dns in $dnsList) {
        $altNamesBlock += "DNS.$i = $dns`n"
        $i++
    }
    $i = 1
    foreach ($ip in $ipList) {
        $altNamesBlock += "IP.$i = $ip`n"
        $i++
    }
...
"@
    ...
    $cnfContent | Set-Content -Path $OutputPath -Encoding ascii
    Write-Host "✅ OpenSSL CNF file created: $OutputPath"
    return $OutputPath
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUC9MB5VXHIDWkhxxqfjc56DE1
# iF2ggiT/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWB